...
body	: MSGID message_string MSGSTR message_string
	...
	{
		error(ERR_NO_MSGSTR, $1, cur_po);
		/* NOTREACHED */
	}
	| MSGID message_string MSGID_PLURAL message_string
	{
		error(ERR_NO_MSGSTRS, $1, cur_po);
		/* NOTRECHED */
	}
	| MSGID message_string plural_messages
	{
		error(ERR_NO_MSGID_PLURAL, $1, cur_po);
		/* NOTREACHED */
	}
	;
...
plural_message	: MSGSTR '[' NUM ']' message_string
	{
		struct loc	*pos;
		if ($3 != plural_index) {
			error(ERR_INVALID_PLURALS, $1, cur_po);
			/* NOTREACHED */
		}
		plural_index++;
