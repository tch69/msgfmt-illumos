...
invoke_gnu_msgfmt(void)
{
	...
	if (verbose) {
		diag(DIAG_INVOKING_GNU);
	}

	(void) execv(gnu_msgfmt, oargv);
	/* exec failed */
	error(ERR_EXEC_FAILED, gnu_msgfmt);
	/* NOTREACHED */
}
...
/*
 * msgfmt - Generate binary tree using psffm: "Portable
...
read_psffm(char *file)
{
	...
	if (fstat(fd, &statbuf) == -1) {
		error(ERR_STAT_FAILED, filename);
		/* NOTREACHED */
	}
	...
	if (addr == MAP_FAILED) {
		error(ERR_MMAP_FAILED, filename);
		/* NOTREACHED */
	}
	(void) close(fd);
	...
	for (;;) {
		...
		if (ln_size == (size_t)-1) {
			error(ERR_READ_FAILED, filename);
			/* NOTREACHED */
		} else if (ln_size == 0) {
			break;	/* End of File. */
		}
		...
		if (token_found || (quotefound && inmsgid)) {
			...
			if (inmsgid && !quotefound) {
				warning(WARN_NO_MSGSTR, msgid_linenum);
				continue;
			}
			if (inmsgstr) {
		...
		if (token_found || (quotefound && inmsgstr)) {
			...
			if (inmsgstr && !quotefound) {
				warning(WARN_NO_MSGID, msgstr_linenum);
				continue;
			}
			if (inmsgstr) {
...
load_buffer:
		...
		if (!bufptr) {
			warning(WARN_SYNTAX_ERR, linenum);
			continue;
		}

		if (*linebufptr++ != L'\"') {
			warning(WARN_MISSING_QUOTE, linenum);
			--linebufptr;
		}
		quotefound = 0;
		...
		while (wc = *linebufptr++) {
			...
			case L'\n':
				if (!quotefound) {
warning(WARN_MISSING_QUOTE_AT_EOL, linenum);
				}
				break;
	...
	if (munmap(addr, statbuf.st_size) == -1) {
		error(ERR_MUNMAP_FAILED, filename);
		/* NOTREACHED */
	}
...
output_one_mo_file(struct domain_struct *dom)
{
	 ...
	 * The middle one is the first message to check.
	 */
	i = (message_count - 1) / 2;
