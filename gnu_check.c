...
check_format(struct entry *id, struct entry *str, int is_c_format)
{
	...
	if (!plural) {
		...
		for (i = 0; i < str->no; i++) {
			...
			if (id_b_newline && !str_b_newline) {
				diag(ERR_BEGIN_NEWLINE_5,
					id_num, p->num, cur_po, i);
				po_error++;
			} else if (!id_b_newline && str_b_newline) {
				diag(ERR_BEGIN_NEWLINE_6,
					id_num, p->num, cur_po, i);
				po_error++;
			}

			if (id_e_newline && !str_e_newline) {
				diag(ERR_END_NEWLINE_5,
					id_num, p->num, cur_po, i);
				po_error++;
			} else if (!id_e_newline && str_e_newline) {
				diag(ERR_END_NEWLINE_6,
					id_num, p->num, cur_po, i);
				po_error++;
			}
		}
	...
	if (id_fmt == -1) {
		diag(ERR_INVALID_FMT, id_num, cur_po);
		po_error++;
	}
	...
	if (!plural) {
		...
		if (str_fmt == -1) {
			diag(ERR_INVALID_FMT, str_num, cur_po);
			po_error++;
		}
	...
	if (plural_fmt == -1) {
		diag(ERR_INVALID_FMT, plural_num, cur_po);
		po_error++;
	}
